...
| `processed_dir` | thư mục `data/processed/versions/<mã>` (nơi ghi dữ liệu đã xử lý) |
| `dataset` | cấu hình dataset: `name`, `aspects`, `labels`, `text_column`, `drop_columns`, `splits`, `_raw_dir`… |
| `pipeline_config` | nội dung `configs/pipeline.yaml` (đã bổ sung giá trị mặc định) |
...
([02_eda/03_modules.md §3](../02_eda/03_modules.md)), nên định nghĩa "nhiễu" của hai
bên giống nhau; chỉ khác thời điểm đo.

> **Khoá so trùng ≠ bước Normalize.** Khoá so trùng bỏ dấu câu (và bỏ dấu tiếng Việt
> khi `ignore_diacritics: true` — config đang để `false`); nó **không bao giờ sửa văn
> bản**, chỉ dùng để *so*. Bước Normalize (Step 4) mới là bước sửa văn bản, và nó
...
> [02_eda/02_metrics.md §5](../02_eda/02_metrics.md).
...
chi tiết: [03_config.md §3](03_config.md).
...
[02_eda/02_metrics.md §12](../02_eda/02_metrics.md).
...
không bị chạm, xem [04_invariants.md §1](04_invariants.md)); các file số liệu về mức
...
> thì khoá đó cũng chỉ dùng để *so*, không sửa văn bản.
...
[02_eda/02_metrics.md §8](../02_eda/02_metrics.md).
...
cạnh nào** (mọi ô = `0`) — xem [02_eda/02_metrics.md §11](../02_eda/02_metrics.md).
...
| **Nhãn không bị đổi** | dấu vân tay nhãn trước / sau chuẩn hoá phải trùng — [04_invariants.md §1](04_invariants.md) |
| **Văn bản chỉ đổi hình thức** | từng dòng đầu ra phải khớp với dòng gốc sau khi áp lại `normalize_steps` (so từng ký tự) — [04_invariants.md §2](04_invariants.md) |
...
mục "Phân biệt ba loại kiểm tra" ở [01_flow.md §6](01_flow.md).
...
mục lục.
Ra (trong `data/processed/versions/<mã>/`): `processed_train.csv`, `processed_val.csv`,
`processed_test.csv`, `label_map.json`, `processing_log.json`; và một dòng mới trong

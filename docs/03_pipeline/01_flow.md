...
[02_eda/01_flow.md §7](../02_eda/01_flow.md).
...
PROCESSED DATA (data/processed/versions/<mã>/)
```
...
|------|------|--------|----------|
| 1. Load | `src/pipeline/load.py` | Nạp dữ liệu qua loader (CSV đọc với `utf-8-sig`), bỏ cột thừa, đổi tên cột văn bản thành `text` | [02_steps.md §1](02_steps.md) |
| 2. Validate | `src/pipeline/validate.py` | Kiểm tra cột thiếu/thừa, review rỗng, nhãn lạ. **Ghi nhận, không sửa** | [02_steps.md §2](02_steps.md) |
| 3. Clean | `src/pipeline/clean.py` | Loại nhiễu & trùng lặp, cách ly xung đột nhãn, xử lý rò rỉ dữ liệu | [02_steps.md §3](02_steps.md) |
| 4. Normalize | `src/pipeline/normalize.py` | Unicode, khoảng trắng, ký tự lặp (tuỳ chọn). **Không thay teencode** | [02_steps.md §4](02_steps.md) |
| 5. Transform | `src/pipeline/transform.py` | Bảng multi_head (mã nhãn 0/1/2/3) + bản ghi ABSA | [02_steps.md §5](02_steps.md) |
| 6. Final Validate | `src/pipeline/final_validate.py` | Schema, text rỗng, nhãn hợp lệ, **nhãn không bị đổi**, số dòng khớp | [02_steps.md §6](02_steps.md) |
...
Mọi phép biến đổi đều **bật / tắt được** trong `configs/pipeline.yaml`. Bảng dưới
...
| 3 | `clean.deduplicate.ignore_diacritics` | **Tắt** | khoá so trùng **giữ dấu** tiếng Việt (`son dep` ≠ `son đẹp`) |
...
- bảng **"Cấu hình đã dùng cho lần chạy này"** (cuối báo cáo) — in toàn bộ
  `configs/pipeline.yaml`, và config nguyên vẹn cũng được ghi vào
...
[02_metrics.md §12](../02_eda/02_metrics.md).
...
| File | Nội dung |
|------|----------|
| `data/reports/pipeline/versions/<mã>/report.html` | **báo cáo duy nhất cho người đọc** |
| `data/processed/versions/<mã>/processed_train.csv` … | dữ liệu đã xử lý |
| `data/processed/versions/<mã>/label_map.json` | danh sách khía cạnh + mã nhãn |
| `data/processed/versions/<mã>/processing_log.json` | config + số liệu để truy vết |
| `data/processed/manifest.json` | mục lục mọi phiên bản đã chạy |
| `data/reports/pipeline/versions/<mã>/removed_records.csv` | danh sách dòng bị loại |
| `data/reports/pipeline/versions/<mã>/quarantine_records.csv` | danh sách dòng bị cách ly |
...
|----|-------|
| `0` | chạy xong |
| `1` | không có gì để vẽ (chưa chạy pha đó, hoặc phiên bản không có file kết quả) |
...
| Loại | Ở đâu | Mục đích |
|------|-------|----------|
| **EDA** | Pha riêng, `src/eda/` | Hiểu dữ liệu: "Có bao nhiêu dòng trùng?" |
...
- **không viết lại teencode** — không có công tắc nào cho việc này trong
  `configs/pipeline.yaml`; việc nhận diện teencode chỉ để ĐO (EDA 03);

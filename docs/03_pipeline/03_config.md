# Cấu hình pipeline (`configs/pipeline.yaml`)
...
- Trạng thái **đang chạy** (bảng tóm tắt) nằm ở [01_flow.md §3](01_flow.md). File này
...
Cơ chế chạy của hai giá trị này (gom nhóm, so nhãn, ghi file): [02_steps.md §3](02_steps.md).
...
Đây là lý do chính khiến mọi phép đều có công tắc. Ví dụ muốn biết
...
```bash
python run_pipeline.py      # -> data/processed/versions/cosmetics-v0.1.0-<hash A>/
```
...
```bash
python run_pipeline.py      # -> data/processed/versions/cosmetics-v0.1.0-<hash B>/
```
...
> `configs/datasets/<tên>.yaml`) khi thay đổi cấu hình quan trọng, để tên phiên bản
> dễ đọc hơn (mã hash vẫn luôn khác nhau kể cả khi bạn quên tăng). Không cần copy
> thư mục `data/processed/` bằng tay nữa.

Xem thêm: [04_experiments/03_training_eval.md](../04_experiments/03_training_eval.md) §3
— danh sách câu hỏi thực nghiệm ứng với từng công tắc.

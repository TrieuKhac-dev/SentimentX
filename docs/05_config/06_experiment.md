...
Tệp: `experiments/<model_id>/<method>/<expNNN>/config.yaml`.
...
| `prompt` | đường dẫn tệp prompt, tính từ gốc repo |
| `examples` | đường dẫn tệp ví dụ few-shot |
| `system_prompt` | đường dẫn tệp chứa khối `[SYSTEM]`; bắt buộc nếu prompt dùng ô nhớ `{system_prompt}` |
...
Ngoài ra: giá trị của mỗi vai phải là khoá có thật trong `splits` của tệp dataset version.
...
`configs/datasets/<name>/<v>.yaml`, và các tệp prompt. Chúng được ghi vào `run_meta.files[]` kèm `role`.

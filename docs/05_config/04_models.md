...
Tệp: `configs/models/<model_id>.yaml`.

Tệp này chứa **mọi cấu hình dùng chung cho model đó**, tức mọi thứ không thuộc riêng một thí nghiệm.
...
| Nhóm | Khoá | Ý nghĩa |
|---|---|---|
| định danh | `model_id` | tên dùng cho mọi đường dẫn và nhãn MLflow; trùng tên tệp |
| định danh | `checkpoint` | tên model trên Hugging Face, để đối chiếu tránh nhầm model |
| định danh | `config_version` | tăng mỗi khi sửa tệp này |
| bài toán | `task.*` | ghi đè `configs/experiments/task.yaml` khi model này cần khác, ví dụ model chỉ làm 2 nhãn |
...
2. Thêm tệp YAML trong `configs/models/` với `model_id` trùng tên tệp.

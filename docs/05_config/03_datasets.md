...
Tệp: `configs/datasets/<name>/<version>.yaml`. Mỗi phiên bản một tệp, **bất biến**.
...
| `version` | nhãn phiên bản dataset, trùng tên tệp |
...
| `splits` | tên tệp của ba split |
| `full` | tệp gộp toàn bộ dữ liệu, tuỳ chọn |
...
Tệp phiên bản đã dùng thì không sửa. Guard so `sha256` của tệp với giá trị đã ghi trong mọi

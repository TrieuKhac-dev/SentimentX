...
EDA chạy trên dữ liệu gốc hoặc trên dataset, và kết quả nằm ngay cạnh thứ được đo:

- Đo trên raw: `data/raw/<name>/<raw_version>/eda/`.
- Đo trên dataset: `data/processed/<mã>/eda/`.

EDA không sửa dữ liệu ở cả hai trường hợp.
...
## 3. Config và ngưỡng đang áp dụng

EDA **không có config bật/tắt riêng**: nó là phép đo, luôn chạy đủ 5 mục. Các
tham số dưới đây quyết định _đo như thế nào_; giá trị ghi ở đây là giá trị **đang
dùng trong repo**:

| Tham số                               | Nơi khai báo                     | Giá trị hiện tại                                                                 | Ảnh hưởng                                                                                                                                                                            |
| ------------------------------------- | -------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `clean.deduplicate.ignore_diacritics` | `configs/pipeline/v0.1.0.yaml`   | **`false`** (TẮT)                                                                | khoá so trùng của EDA 03 và EDA 05 **giữ dấu tiếng Việt** - đúng bằng khoá mà pipeline dùng, nên hai bên không bao giờ lệch nhau. Đây là **config duy nhất** của pipeline mà EDA đọc |
...
| File                                                | Nội dung                                                   |
| --------------------------------------------------- | ---------------------------------------------------------- |
| `data/raw/<name>/<raw_version>/eda/report.html`     | **báo cáo duy nhất cho người đọc** - mở bằng trình duyệt   |
| `data/raw/<name>/<raw_version>/eda/eda_result.json` | số liệu mà báo cáo đọc lại (đủ 5 phần)                     |
| `data/raw/<name>/<raw_version>/eda/0X_<phần>.json`  | file kết quả riêng của từng phần, ví dụ `01_overview.json` |
...
```
1. Raw -> EDA -> hiểu dữ liệu -> chốt policy
2. Raw -> Pipeline -> dataset
3. (nếu cần) dataset -> Data Quality Report
```

`Data Quality Report` ở tiền xử lý cho model **không phải EDA lần hai** - nó là **cổng chất lượng**
...
[03_pipeline/02_steps.md](../03_pipeline/02_steps.md) mục 6.

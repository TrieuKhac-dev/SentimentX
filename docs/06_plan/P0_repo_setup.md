...
- `data/models/Qwen3-4B-Instruct-2507/` và `data/raw/cosmetics/` vẫn còn trên đĩa.

## 5. Rủi ro / lưu ý

- `git push` có thể cần xác thực GitHub. Nếu terminal chưa đăng nhập, dừng ở commit local và báo lại.
- Xoá `.git` cây cũ là không thể hoàn tác. Cây cũ vẫn giữ tệp làm tham chiếu, chỉ mất lịch sử git.

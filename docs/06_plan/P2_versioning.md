...
Dữ liệu có phiên bản rõ ràng: raw, dataset, pipeline đều có mã; tệp phiên bản là bất biến và có guard.
...
- [ ] T1. Tách `configs/pipeline.yaml` thành `configs/pipeline/<version>.yaml`.
      → `refactor(config): split pipeline config into versioned files`
- [ ] T2. Tách `configs/datasets/cosmetics.yaml` thành `configs/datasets/cosmetics/<version>.yaml`
...
- [ ] T4. Guard bất biến: so `sha256` của tệp phiên bản với giá trị đã ghi trong mọi
...
- Sửa tệp phiên bản đã dùng thì lệnh chạy báo lỗi đúng thông điệp đã định.
- `data/processed/<mã>/` chứa `train.csv`, `val.csv`, `test.csv`, `label_map.json`,
  `processing_log.json`, `pipeline/`.

## 5. Rủi ro / lưu ý

- Kết quả cũ trong `data/processed/versions/` và `data/reports/*/versions/` sẽ không dùng nữa.

...
| Đầu ra     | Một thư mục riêng cho mỗi cấu hình: `predictions.csv`, `metrics.json`, `metrics.csv`, `mispredictions.csv`, `run.log` | Tên file cố định (đọc từ `configs/paths.yaml`), cấu hình nằm ở tên thư mục, nên không ghi đè nhau. `metrics.json` ghi `label_space`, `neutral_policy`, số ô neutral bị loại, và `rescored` nếu có chấm lại. `run.log` luôn có; `errors.json` chỉ xuất hiện khi có lỗi (xem [00_workflow/01_flow.md](../00_workflow/01_flow.md)) |

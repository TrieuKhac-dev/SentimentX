...
cấu hình cho ra **một thư mục phiên bản riêng** trong `data/processed/versions/`,
nên ta luôn chỉ ra được khác biệt đến từ phép biến đổi nào:

```bash
python run_pipeline.py        # -> phiên bản A
# sửa một tham số trong configs/pipeline.yaml
...
3. **Thực nghiệm preprocessing** — đổi từng công tắc một (mỗi lần chỉ đổi **một**
...
- Khi so sánh các cấu hình, con số đáng dùng là **số bản ghi ABSA** (card ở Step 5
  ...
  ([02_eda/02_metrics.md §11](../02_eda/02_metrics.md)).
- Trước khi huấn luyện, chạy `python run_token_stats.py` để biết input thật của
  từng model ([02_phase3_input.md §2](02_phase3_input.md)): nếu một model bị cắt quá
  nhiều input thì mọi so sánh sau đó đều không công bằng.

## 6. Kết quả đã có — Qwen3 chạy bằng CHỈ DẪN (prompt một lượt vs CoT)

Đây là phần đầu tiên của pha 4 có số liệu CHẤT LƯỢNG thật. Ba model encoder (PhoBERT,
...
```bash
python run_qwen_eval.py --split val --prompt qwen_absa_v1            --limit 100 --batch-size 4 --model data\models\Qwen3-4B-Instruct-2507 --quiet
python run_qwen_eval.py --split val --prompt qwen_absa_cot_zeroshot_v1 --limit 100 --batch-size 4 --model data\models\Qwen3-4B-Instruct-2507 --quiet
python run_qwen_eval.py --split val --prompt qwen_absa_cot_1shot_v1  --limit 100 --batch-size 4 --model data\models\Qwen3-4B-Instruct-2507 --quiet
python run_qwen_eval.py --split val --prompt qwen_absa_cot_v1        --limit 100 --batch-size 4 --model data\models\Qwen3-4B-Instruct-2507 --quiet
```
...
| Ngưỡng cắt | 1280 token | prompt CoT cần tới 1.195 token (pha 3, §4.2) |
...
Chi phí **input** đã đo ở pha 3 (§4.2): 228,61 / 375,61 / 680,61 / 949,61 token/review — nên
...
| Chấm lại khi đổi cách chấm | `python run_rescore_eval.py` — không cần GPU, vì nhãn đã nằm trong file dự đoán |

---

Xem thêm: [01_models.md](01_models.md) — bốn model;
[02_phase3_input.md](02_phase3_input.md) — chuẩn bị input cho model.

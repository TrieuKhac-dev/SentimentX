...
`data/raw/cosmetics/` với `train`, `val`, `test`, `full` là dữ liệu lấy từ công bố.
...
1. Dùng đúng split `test`, không đổi tập này. Guard là `eval_lock` trong tệp dataset version.
2. Dùng đúng bộ chỉ số trong `docs/04_experiments/metrics.md`.
...
và bộ chỉ số. Mục tiêu là kết quả **nhỉnh hơn** công bố, không chỉ tái hiện.

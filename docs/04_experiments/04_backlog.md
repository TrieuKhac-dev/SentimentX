...
- **Chi phí input đã đo** (train): 229,50 → 376,50 → 681,50 → 950,50 token/review. Mỗi ví
  ...
  mọi split. Bảng đầy đủ ở [02_phase3_input.md §4.2](02_phase3_input.md).
...
- **Hạ tầng chạy model:** `src/evaluation/{parse,metrics,runner}.py` + `run_qwen_eval.py`
...
[03_training_eval.md §6](03_training_eval.md).
...
[02_phase3_input.md §4.1](02_phase3_input.md)): bộ chính chủ cho input ngắn nhất (29,96
...
- **Báo cáo HTML cho pha 3:** `run_token_stats.py` mới ghi CSV + in console, chưa có
  `*_result.json` để `build_report.py` vẽ như EDA/pipeline. Chưa làm vì phép đo còn đang
  thay đổi (prompt, bộ tách từ) nên chưa chốt được bộ chỉ số để trình bày.
...
- **Bài học về test (đã trả giá):** lỗi hoán vị FP/FN trong `metrics._binary_counts` lọt qua
  ...
  **bất đối xứng** (chỉ dương tính giả / chỉ âm tính giả), và con số tổng hợp phải được kiểm
  chéo với số đếm. Kết quả đã ghi được chấm lại bằng `run_rescore_eval.py` (không cần GPU).
...
- **Cơ chế của việc tách từ chưa được lưu thành số liệu:** đã đo trong phiên làm việc là
  ...
  trong [02_phase3_input.md](02_phase3_input.md).

# Pha 3 — chuẩn bị input cho từng model
...
`data/processed/versions/<mã>/`, và sinh ra dạng dữ liệu mà từng model cần
...
| Tên prompt ở `configs/models/*.yaml`, **không** ở `configs/pipeline.yaml` | `pipeline.yaml` được đưa vào hash để sinh **mã phiên bản dữ liệu**, nên để prompt ở đó sẽ đẻ ra mã phiên bản mới vô nghĩa cho cùng một dataset |
...
**Tách từ ≠ tokenizer.** Tách từ gộp các âm tiết của một từ lại ("Đại học Quốc gia" →
...
phụ thuộc vào model nào ([02_eda/02_metrics.md §9](../02_eda/02_metrics.md)).
...
## 5. Cài đặt cho pha 3 (cả nhóm dùng cùng một bản)
...
Mọi model đọc **cùng một nguồn**: `data/processed/versions/<mã>/`. Thư viện
